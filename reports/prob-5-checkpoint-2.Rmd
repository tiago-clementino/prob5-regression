...
O objetivo aqui é criar um relatório analizando a correlação entre o score do professor e sua aparência física. Para isto, precisamos usar técnicas de regressão para analisar a influência de alguns dos fatores presentes nos dados.
 ...
 - **2**. Aqui vou deixar de lado a variável *bty_avg* (beleza) e tentar medir a correlação entre *score* e quatro fatores que julgo possam ser relevantes na direção do requisito beleza física: *cls_level* (nível da disciplina), *age* (idade), *ethnicity* (perfil étnico miniritário ou não) e *gender* (homem ou mulher). À princípio faço quatro regressões simples separadas e observo a correlação entre *score* e cada uma das variáveis. Em seguida comparo com um modelo multivariado envolvendo todas as quatro variáveis. A pergunta que quero responder aqui é: Estes quatro fatores tem mais influência em *score* que *bty_avg* (beleza)?
 

### Mais sobre os dados

Aqui vou apenas observar a "aparência" dos dados. 
...
A principio, a correlação entre beleza e avaliação do professor parece bem tênue.
...
evals %>% 
  ggplot(aes(x = cls_level)) + 
  geom_bar(fill = paleta[1], colour = 'white') + 
    labs(x='Avaliação',  
        y="Densidade", 
        title="Densidade da Distribuição do Score dos Docentes", 
        subtitle="(score, density)", 
...
evals %>% 
  ...
  geom_bar(fill = paleta[3], colour = 'white') + 
    labs(x='bty_avg',  
        y="Densidade", 
        title="Densidade da Distribuição de Beleza Física dos Docentes", 
        subtitle="(bty_avg, density)", 
...
evals %>% 
  ggplot(aes(y = score, x = ethnicity)) + 
  geom_jitter(width = .1, alpha = .3) + 
...
evals %>% 
  ggplot(aes(y = score, x = cls_level)) + 
  geom_jitter(width = .1, alpha = .3) + 
    labs(x='Avaliação',  
        y="Beleza", 
        title="Avaliação X Beleza física", 
